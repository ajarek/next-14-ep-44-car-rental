...
const Car = ({ searchParams }: { searchParams: PropsCar }) => {
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] px-24 max-sm:px-4 py-12 flex flex-col justify-center items-center ">
      <div className="w-full grid grid-cols-2 max-sm:grid-cols-1 gap-8">
        ...
        <div className="flex flex-col gap-4">
          ...
          <p>
            {persons} {Number(persons) > 4 ? 'miejsc' : 'miejsca'}
          </p>
          <p>typ napędu: {gears === 'Manual' ? 'Manualny' : 'Automat'}</p>
          <p>
            Okres wynajmu: od {firstDate} do {secondaryDate}
          </p>
          <p>Ilość dni: {period}</p>
          <p className="capitalize">Miejsce odbioru i zwrotu: {city}</p>
          <p>{Number(price).toFixed(2)} PLN/doba</p>
          <p className="">
            <span className="p">Do zapłaty: </span>
            {(Number(price) * period).toFixed(2)} PLN
          </p>
          <Link
            href={`/checkout?total=${(Number(price) * period).toFixed(2)}`}
